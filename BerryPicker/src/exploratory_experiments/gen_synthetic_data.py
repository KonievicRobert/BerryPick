...
def generate_training_sequence(total_length, latent_size, y_size=5): 
    """
    Generates two sequences of specified length and width with some of the cells being predictable 
    (being based on sine and cosine values etc). 
    The x_seq is going to be used for simple prediction of an evolving stream. 
    The y_seq is going to be used to check whether a different shaped stream that is fully determined by the x_seq can be predicted. 
    """
